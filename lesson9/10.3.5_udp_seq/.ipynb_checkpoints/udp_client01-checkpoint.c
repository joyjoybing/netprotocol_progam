...
static void udpclie_echo(int s, struct sockaddr*to)
{
	...
	for(i=0;i<160;i++)
	{
		*((int*)&buff_send[0])=htonl(i);
		...
		//int err=recvfrom(s,buff,1,0,(struct sockaddr*)&from,&len);
	//	printf("%dst:%c,err:%d\n",i,buff[0],err);
		sendto(s, &buff_send[0], BUFF_LEN, 0, to, len);		/*发送给服务器*/
	}
	//recvfrom(s, buff, BUFF_LEN, 0, (struct sockaddr*)&from, &len);	
												/*从服务器接收数据*/
//	printf("recved:%s\n",buff);					/*打印数据*/
...
static void udpclie_echome(int s, struct sockaddr*to)
{
	...
	for(i=0;i<PACKET_MAX_SEQ-980;i++)
	{
		*((int*)&buff_send[0])=htonl(i);
		memcpy(&buff_send[4],buff,sizeof(buff));
		//printf("i=%d,recved:%s\n",i,buff);	
		//sendto(s,&buff_send[0],PACKET_MAX_SEQ,0,to,len);
        sendto(s,&buff_send[0],BUFF_LEN,0,to,len);
        sleep(1);
        bzero(buff_send,LENGTH);
        recvfrom(s,buff_send,LENGTH,0,to,&len);
        uint32_t seq=ntohl(*((uint32_t*)&buff_send));
...
int main(int argc, char*argv[])
{
	...
	//udpclie_echobak(s, (struct sockaddr*)&addr_serv);	/*出错*/
	//udpclie_echo(s, (struct sockaddr*)&addr_serv);	/*right 客户端回显程序*/
    udpclie_echome(s, (struct sockaddr*)&addr_serv);	/*question 客户端回显程序*/
	close(s);
	return 0;	
}
